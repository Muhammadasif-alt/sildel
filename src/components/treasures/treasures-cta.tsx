...
export function TreasuresCta() {
  const data = useTreasures().content.cta;

  return (
    <section
      id="treasures-cta"
      className="relative w-full bg-muted/30 border-t border-border/60 overflow-hidden"
      aria-labelledby="treasures-cta-heading"
    >
      <div className="mx-auto max-w-[1480px] px-6 py-24 lg:px-12 lg:py-36">
        ...
        <div className="mt-20 lg:mt-28 flex items-center justify-center gap-4" aria-hidden>
...
function DestinationCard({ eyebrow, title, body, image, imageAlt, href, cta }: DestProps) {
  return (
    <Link
      href={href}
      ...
      className={cn(
        ...
        "transition-all duration-300 hover:border-primary hover:shadow-2xl hover:shadow-primary/10",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      )}
    >
      <span aria-hidden className="absolute top-0 left-0 z-20 h-px w-0 bg-primary transition-all duration-500 group-hover:w-full" />
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-muted">
        <DestImage src={image} alt={imageAlt} />
        <div aria-hidden className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/10 transition-colors duration-500" />
      ...
      <div className="p-6 lg:p-10">
        ...
        <span className="inline-flex items-center gap-2 text-xs tracking-[0.35em] uppercase font-medium text-foreground transition-colors group-hover:text-primary">
          {cta}
          <span aria-hidden className="h-px w-6 bg-current transition-all duration-300 group-hover:w-10" />
        </span>
      </div>
    </Link>
  );
...
function DestImage({ src, alt }: { src: string; alt: string }) {
