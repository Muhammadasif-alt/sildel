...
export function YtHero() {
  const data = useYouThinkCork().hero;

  return (
    <section
      id="yt-hero"
      className="relative w-full overflow-hidden isolate"
      aria-labelledby="yt-hero-heading"
    >
      <YtHeroBackground src={data.image} alt={data.imageAlt} />
      <div aria-hidden className="absolute inset-0 bg-gradient-to-b from-black/55 via-black/40 to-black/75" />
      <div aria-hidden className="absolute inset-0 bg-gradient-to-r from-black/60 to-transparent" />
...
function YtHeroBackground({ src, alt }: { src: string; alt: string }) {
